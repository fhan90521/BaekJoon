...
int main()
{
	...
	{
		sTreeVec[i] = MIN(sTreeVec[i * 2],sTreeVec[i * 2 + 1]);
	}
	...
	for (int i = 0; i < M; i++)
	{
		...
		long long start_index = size / 2 + a - 1;
		long long end_index = size / 2 + b - 1;
		while (start_index <= end_index)
		{
			if (start_index % 2 == 1)
			{
				min = MIN(min,sTreeVec[start_index]);
			}
			start_index = (start_index + 1) / 2;
			if (end_index % 2 == 0)
			{
				min = MIN(min, sTreeVec[end_index]);
			}
