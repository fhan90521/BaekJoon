...
int main()
{
	...
	sTreeVec = vector<long long>(size, 1);
	...
	{
		sTreeVec[i] = (sTreeVec[i * 2] * sTreeVec[i * 2 + 1])% DIVIDE_VAL;
	}
	...
	for (int i = 0; i < M + K; i++)
	{
		...
		if (a == 1)
		{
			...
			while (index >= 1)
			{
				sTreeVec[index] = (sTreeVec[index * 2] * sTreeVec[index * 2 + 1]) % DIVIDE_VAL;
				index /= 2;
			}
		}
		else if (a == 2)
		{
			...
			while (start_index <= end_index)
			{
				if (start_index % 2 == 1)
				{
					answer = (answer*sTreeVec[start_index])% DIVIDE_VAL;
				}
				start_index = (start_index + 1) / 2;
				if (end_index % 2 == 0)
				{
					answer = (answer * sTreeVec[end_index]) % DIVIDE_VAL;
				}
